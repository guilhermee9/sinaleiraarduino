...
//saida
int sinal_verm_est = 2;
int sinal_verd_est = 9;
int sinal_verm_ramp = 4;
int sinal_verd_ramp = 3;
...
void setup()
{
  ...
  digitalWrite(prox_est, HIGH);
  digitalWrite(prox_ramp, HIGH);
  digitalWrite(prox_ent, HIGH);
  digitalWrite(prox_e_ramp, HIGH);
}
...
void loop()
{
  if (digitalRead(totem_ent) == 1) {
    ...
    setRelay(LOW,LOW,LOW,HIGH,LOW,LOW,LOW);
    delayAuto();
  }else if (digitalRead(totem_e_ramp) == 1) {
    alterado = 1;
    msgBotao = "Totem Entrada Rampa";
    setRelay(HIGH,LOW,HIGH,LOW,LOW,LOW,LOW);
    delayAuto();
  }
}
